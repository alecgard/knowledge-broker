...
func TestHandleHistory_EmptyList(t *testing.T) {
	st := newTestStore(t)
	emb := &mockEmbedder{dim: testEmbeddingDim}
	srv := NewHTTPServer(nil, emb, st, nil)
	...
	var entries []store.QueryHistoryEntry
...
func TestHandleHistory_SaveAndList(t *testing.T) {
	st := newTestStore(t)
	emb := &mockEmbedder{dim: testEmbeddingDim}
	srv := NewHTTPServer(nil, emb, st, nil)
	...
	entry := store.QueryHistoryEntry{
		...
		Query:        "what is auth?",
		Mode:         "local",
		ResponseJSON: `{"content":"auth is..."}`,
		LatencyMS:    150,
	}
	body, _ := json.Marshal(entry)
	req := httptest.NewRequest(http.MethodPost, "/v1/history", bytes.NewReader(body))
	rec := httptest.NewRecorder()
	srv.Handler().ServeHTTP(rec, req)
	...
	if entries[0].Query != "what is auth?" {
		t.Fatalf("expected query 'what is auth?', got %q", entries[0].Query)
	}
	if entries[0].LatencyMS != 150 {
		t.Fatalf("expected latency 150, got %d", entries[0].LatencyMS)
	}
}

func TestHandleHistory_SaveValidation(t *testing.T) {
	st := newTestStore(t)
	emb := &mockEmbedder{dim: testEmbeddingDim}
	srv := NewHTTPServer(nil, emb, st, nil)

	// Empty query should fail.
	entry := store.QueryHistoryEntry{ID: "test1", Query: "", Mode: "local"}
	body, _ := json.Marshal(entry)
	req := httptest.NewRequest(http.MethodPost, "/v1/history", bytes.NewReader(body))
	rec := httptest.NewRecorder()
	srv.Handler().ServeHTTP(rec, req)
	...
	entry = store.QueryHistoryEntry{ID: "test2", Query: strings.Repeat("x", 10001), Mode: "local"}
	body, _ = json.Marshal(entry)
	req = httptest.NewRequest(http.MethodPost, "/v1/history", bytes.NewReader(body))
	rec = httptest.NewRecorder()
	srv.Handler().ServeHTTP(rec, req)
...
func TestHandleHistory_DeleteEntry(t *testing.T) {
	st := newTestStore(t)
	emb := &mockEmbedder{dim: testEmbeddingDim}
	srv := NewHTTPServer(nil, emb, st, nil)
	...
	if rec.Code != http.StatusCreated {
		t.Fatalf("save: expected 201, got %d", rec.Code)
	}

	// Delete by ID.
	req = httptest.NewRequest(http.MethodDelete, "/v1/history/del1", nil)
	rec = httptest.NewRecorder()
	srv.Handler().ServeHTTP(rec, req)
...
func TestHandleHistory_ClearAll(t *testing.T) {
	st := newTestStore(t)
	emb := &mockEmbedder{dim: testEmbeddingDim}
	srv := NewHTTPServer(nil, emb, st, nil)
	...
	for _, id := range []string{"c1", "c2"} {
		entry := store.QueryHistoryEntry{ID: id, Query: "q " + id, Mode: "local", ResponseJSON: "{}"}
		body, _ := json.Marshal(entry)
		req := httptest.NewRequest(http.MethodPost, "/v1/history", bytes.NewReader(body))
		rec := httptest.NewRecorder()
		srv.Handler().ServeHTTP(rec, req)
		if rec.Code != http.StatusCreated {
			t.Fatalf("save %s: expected 201, got %d", id, rec.Code)
		}
	}
...
func TestHandleHistory_MethodNotAllowed(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPut, "/v1/history", nil)
	rec := httptest.NewRecorder()
	srv.Handler().ServeHTTP(rec, req)
